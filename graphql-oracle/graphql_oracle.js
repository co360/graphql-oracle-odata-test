...
let dbConfig = {
  ...
  user: 'DBUSER',
  password: 'DBPW',
  //connectString: 'jdbc:oracle:thin:DBUSER/DBPW@127.0.0.1:49161:xe.oracle.docker'
  connectString:
    '( DESCRIPTION=( ADDRESS=(PROTOCOL=tcp) (HOST=127.0.0.1) (PORT=49161) )( CONNECT_DATA=(SERVICE_NAME=xe.oracle.docker) ) )'
};
...
async function getAllEntities() {
  let sql =
    'SELECT ENTITY_ID, ENTITY_NAME, BARCODE FROM ENTITY where rownum <= 200';
  let conn = await oracledb.getConnection();
  let result = await conn.execute(sql);
  await conn.close();
